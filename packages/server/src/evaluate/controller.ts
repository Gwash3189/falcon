...
export function createEvaluateController(redis: Redis) {
  return {
    async evaluate(c: Context) {
      ...
      if (cached !== null) {
        ...
        return c.json({
          data: { flag_key, enabled: evaluateFlag(flag, identifier) },
        });
      }

      const flag = await getFlagByKey(environmentId, flag_key);
      if (!flag) {
